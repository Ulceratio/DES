...
{
    class BitArray: ICollection<bool>,ICloneable,IEnumerable
    {
        ...
        public BitArray(IEnumerable<byte> inputByteList,int byteLength)
        {
            ...
            Func<byte, List<bool>> transformByteToListBools = new Func<byte, List<bool>>((element) => {
                ...
                for (int i = 0; i < byteLength; i++)
                {
                    tempList[i] = FirstSectionOfOperations.getIbit(i, element) == 1 ? true : false;
                }
                tempList.Reverse();
                return tempList;
            });
        ...
        private void getBitArrayFromByteArr(List<byte> inputArray)
        {
            intraBitArray = new List<bool>();
            Func<byte, List<bool>> transformByteToListBools = new Func<byte, List<bool>>((element) => {
                List<bool> tempList = new List<bool>() { false, false, false, false, false, false, false, false };
                for (int i = 0; i < 8; i++)
                {
                    tempList[i] = FirstSectionOfOperations.getIbit(i, element) == 1 ? true : false;
                }
        ...
        public static BitArray operator <<(BitArray bitArray, int n)
        {
            bool tempval = false;
            for (int i = 0; i < n; i++)
            {
        ...
        public static class FirstSectionOfOperations
        {
            ...
            public static int getIbit(int IndexNumber, int Number)
            {
